...
[0.25-d1, 0.25-d2, 0.0+d3], #I
[0.75-d1, 0.25+d2, 0.0+d3], #I
[0.75+d1, 0.75+d2, 1.0-d3], #I
[0.25+d1, 0.75-d2, 1.0-d3], #I

[0.25-d1, 0.25-d2, 0.5-d3], #I
[0.75-d1, 0.25+d2, 0.5-d3], #I
[0.75+d1, 0.75+d2, 0.5+d3], #I
[0.25+d1, 0.75-d2, 0.5+d3], #I

[0.00+d4, 0.50+d5, 0.2500], #I
[0.50+d4, 1.00-d5, 0.2500], #I
[1.00-d4, 0.50-d5, 0.7500], #I
[0.50-d4, 0.00+d5, 0.7500]])#I
...
xmin = -lxy * (math.floor(nx/2.0)+0.2)
xmax =  lxy * (math.ceil(nx/2.0)+0.2)

ymin = -lxy * (math.floor(ny/2.0)+0.2)
ymax =  lxy * (math.ceil(ny/2.0)+0.2)
		...
		for z_cell in range(-1,int(nz)+1):
				for i,atom in enumerate(atoms):
					...
					if pos[0]>=xmin and pos[0]<=xmax and pos[1]>=ymin and pos[1]<=ymax and pos[2] >= zmin and pos[2]<=zmax:
						print(f"{atom} {pos[0]:.6f} {pos[1]:.6f} {pos[2]:.6f}")
...
for i in range(natoms):
	if not(i in to_remove):
		filter_of.write(f"{built_dot[i][0]} {built_dot[i][1]:.6f} {built_dot[i][2]:.6f} {built_dot[i][3]:.6f}\n")
filter_of.close()
...
for i in range(natoms):
	...
	if not(i in to_remove):
		...
		lammps_of.write(f"{i+1} {i+1} {atom_type+1} {atom_charges[atom_type]} {xpos[i]:.6f} {ypos[i]:.6f} {zpos[i]:.6f}\n")
...
for i in range(natoms):
	if not(i in to_remove):
		xyz_of.write(f"{built_dot[i,0]} {xpos[i]:.6f} {ypos[i]:.6f} {zpos[i]:.6f}\n")
xyz_of.close()
